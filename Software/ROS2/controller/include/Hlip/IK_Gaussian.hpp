...
public:
    ...
    void solve_from_current(const VectorXd &q, const VectorXd &Ydes, const VectorXd &dYdes)
    {
        ...
        // std::cout<< "Solved, somehow" <<std::endl;
    }
...
private:
    ...
    {           
        ...
        for (unsigned int i = 0; i < max_iteration; ++i)
        {
            ...
            // std::cout << "new_qx: " << qx_iterating.transpose() << " res_err: " <<  residual_error << std::endl;
            ...
            if (residual_error < error_residual_toleance)
            {
                ...
                // std::cout << "output_q: " << qx_iterating.transpose() << std::endl;
                return i;
            }

            J_error_evaluation(J_error);
            VectorXd g = J_error.transpose()*this->We*error_function_value;
            MatrixXd H = J_error.transpose()*this->We*J_error;
            qx_iterating = qx_iterating - (H.inverse()) * (g);
            
        }
        string error = this->ik_name + " IK Gaussian DID NOT CONVERGE WITH ERROR" + to_string(residual_error) + "\n";
        // std::cout << error;
