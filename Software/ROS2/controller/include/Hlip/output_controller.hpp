...
class output_controller: public HLIP{
...
public:
    ...
    // IK_Newton_Solver output_controller_IK_solver; 
    IK_Gaussian_Solver output_controller_IK_solver;
    // IK_LM_Solver output_controller_IK_solver;

    // before walking
    // we set the initial pose for all states to be 0
    // then directly use HLIP to plan a trajectory
    void output_controller_initialize(orbit_type orbit, double vdes, double COM_height, double TSSP, double TDSP, double step_size, double swing_height,double torso_angle);
...
// update the states of the robots
    ...
    void output_controller_renew(VectorXd state_q_real, VectorXd state_dq_real, VectorXd output_state_real, VectorXd doutput_state_real, stance_leg stance_leg_input, Vector2d COM_x,double T);
...
// 1. plan swing foot z trajectory 
    void output_controller_swing_foot_z(); 
// 2. plan swing foot x trajectory 
    void output_controller_swing_foot_x(); 
// 3. plan COM z trajectory
    void output_controller_COM_z(); 
//4. plan COM x trajectory (only for doing IK)
    void output_controller_COM_x();
// construct all the trajectory in one go
    void output_controller_plan_desired();


/////////////// joint level PD /////////////
// use IK to find desired joint angles and velocities
    void output_controller_IK(); 
    MatrixXd output_controller_J_output(VectorXd q);
    MatrixXd output_controller_y_output(VectorXd q);
// task-space PD for desired torque
    VectorXd output_controller_output_PD(VectorXd q_real, VectorXd dotq_real, VectorXd q_des, VectorXd dotq_des, MatrixXd Jacob, MatrixXd K_p, MatrixXd K_d);
    MatrixXd output_controller_construct_J(VectorXd q);

// gravity compnesation
    VectorXd output_controller_gravity_compensation(VectorXd q); 
