...
namespace mujoco
{
  ...
  public:
    ...
    void renderloop(bool if_record_video, std::string video_name, int width, int height);

    // constants
    static constexpr int kMaxFilenameLength = 1000;

    // rendoring 
    void init_recording( const char *fileName, int width, int height);
    void record_frame(); 
    ...
    // additional stuff for video recording
    int video_width;
    int video_height;
    unsigned char *frame;
    FILE *pipe_video_out;
