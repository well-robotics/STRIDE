...
#define ROBOT_VIDEO_FRAME_RATE "30"
...
namespace mujoco
{
  ...
  void Simulate::render()
  {
    ...
    mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, smallrect,
                str_paused,
                str_status, &this->platform_ui->mjr_context());
  ...
  void Simulate::renderloop(bool if_record_video, std::string video_name, int width, int height)
  {
    ...
    if (if_record_video)
    {
      this->init_recording(video_name.c_str(), width, height);
    }
  ...
  // TRY recording video
  void Simulate::init_recording(const char *fileName, int width, int height)
  {
    ...
    strcat(ffmpeg_cmd, ROBOT_VIDEO_FRAME_RATE); // Frame Rate
    ...
    this->platform_ui->SetWindowSize(width, height);

    this->frame = (unsigned char *)malloc(3 * width * height);

    this->pipe_video_out = popen(ffmpeg_cmd, "w");
  }

  void Simulate::record_frame()
  {
    std::pair<int, int> windowSizeTemp;
    mjrRect viewport = {0, 0, 0, 0};
    windowSizeTemp = this->platform_ui->GetFramebufferSize();
    viewport.width = windowSizeTemp.first;
    viewport.height = windowSizeTemp.second;

    // This checks if the window is resized and loops until it is released and corrected
    if (viewport.width != this->video_width || viewport.height != this->video_height)
    {
      while (viewport.width != this->video_width || viewport.height != this->video_height)
      {
        windowSizeTemp = this->platform_ui->GetWindowSize();
        this->video_width = windowSizeTemp.first;
        this->video_height = windowSizeTemp.second;

        windowSizeTemp = this->platform_ui->GetFramebufferSize();
        viewport.width = windowSizeTemp.first;
        viewport.height = windowSizeTemp.second;
        usleep(10000);
        if (!this || !this->platform_ui->window_)
          return;
      }
    }
    else
    { // Normal case where the right size so it can render to file

      this->platform_ui->getContext(this->frame, viewport);
      // Write frame to output pipe

      fwrite(this->frame, 1, this->video_width * this->video_height * 3, this->pipe_video_out);
    }
  }

  void Simulate::close_recording()
  {
    if (this->pipe_video_out)
    {
      fflush(this->pipe_video_out);
      pclose(this->pipe_video_out);
      this->pipe_video_out = NULL;
      free(this->frame);
    }
  }
} // namespace mujoco
